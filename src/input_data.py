...
def gather_exc_energies(method, basis_set_mexc, nStates, acquiredStates, T):
    ...
    #os.chdir('../calc_zone')
    boltzmannAnalysisSetup(assume_complete, method, basis_set_mexc, nStates,
                           acquiredStates)
    boltzmannAnalysis(T)
    print(os.getcwd())
    #os.chdir("../src")
...
def data_to_specsim(data, x_range=[6, 12], deltaG=1):
    ...
    cmd = 'perl specsim_args.pl %.2f %.2f %.2f' % (x_range[0], x_range[1],
                                                   deltaG)
...
def gas_solid_plot_h2o():
    mol = 'h2o'
    exp_solid = np.genfromtxt('../../../exp_data/%s_solid.csv' % mol,
                              delimiter=', ')
    exp_gas = np.genfromtxt('../../../exp_data/%s_gas.csv' % mol,
                            delimiter=', ')
    ...
    data_lst_comp = [[mon, 'Monomer', '-'],
                     [rand32, '32 Randomized Clusters', '-']]
    data_lst_exp = [[exp_gas, 'Exp. Gas', '--'],
                    [exp_solid, 'Exp. Solid', '--']]
