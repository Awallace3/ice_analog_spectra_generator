...
def add_imaginary(freq_clean, freq_lst_len, filename):
    ...
    """    carts = carts.astype(str)
        carts = carts.tolist() """
    np.savetxt("tmp.txt", carts, fmt="%s")
...
def find_geom(lines, error, filename, imaginary):
    ...
    out_file = "tmp.txt"
    np.savetxt(out_file, new_geom, fmt="%s")
    if not imaginary:
        clean_many_txt()
    elif error:
        clean_many_txt()
...
def make_input_files_no_constraints(
    output_num, method_opt, basis_set_opt, mem_com_opt, mem_pbs_opt, SCRF=""
):
...
def make_mexc(
    method_mexc, basis_set_mexc, mem_com_mexc, mem_pbs_mexc, nStates="25", SCRF=""
):
    """
    Combines the geometry output and the constrained output. Then makes the .com and .pbs files in a subdirectory
    """
...
def main(
    ...
    if len(out_files) > 0:
        ...
        # try:
        if error == True:
            find_geom(lines, error=True, filename=filename, imaginary=imaginary)
            ...
            find_geom(lines, error=False, filename=filename, imaginary=imaginary)
